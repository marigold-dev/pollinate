...
let from_sockaddr =
  QCheck2.Test.make ~count:1000
    ~name:"from_sockaddr from any sockaddress is successful"
    Generators.sockaddress_gen (fun sockaddress ->
...
let to_sockaddr =
  QCheck2.Test.make ~count:1000
    ~name:"to_sockaddr from any address is successful" Generators.address_gen
    (fun address ->
