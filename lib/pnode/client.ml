...
let create_request node recipient ~sign_payload:sign_payload ~key:key payload =
...
let create_response node request ~sign_payload:sign_payload ~key:key payload =
  Message.
    {
...
let request node ~sign_payload:sign_payload ~key:(key : bytes option) request recipient =
  let%lwt message = create_request node recipient ~sign_payload:sign_payload ~key:key request in
...
let broadcast_request node recipients ~sign_payload:sign_payload ~key:(key : bytes option) req =
  List.map (request node ~sign_payload:sign_payload ~key:key req) recipients
