...
val create_request : node ref -> Address.t -> (bytes -> bytes option -> bytes option) -> bytes option -> bytes-> Message.t Lwt.t
...
val create_response : node ref -> Message.t -> (bytes -> bytes option -> bytes option) -> bytes option -> bytes -> Message.t
...
arrives. *)
val request : node ref -> bytes -> (bytes -> bytes option -> bytes option) -> bytes option -> Address.t -> Message.t Lwt.t
...
val broadcast_request :
  node ref -> bytes -> Address.t list -> (bytes -> bytes option -> bytes option) -> bytes option -> Message.t Lwt.t list
